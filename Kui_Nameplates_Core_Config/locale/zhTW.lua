local L = KuiNameplatesCoreConfig:Locale('zhTW')
if not L then return end

L["common"] = {
	["copy"] = "複製",
	["delete"] = "刪除",
	["font_size"] = "文字大小",
	["height"] = "高度",
	["layout"] = "佈局",
	["offset"] = "偏移量",
	["offset_x"] = "水平偏移量",
	["offset_y"] = "垂直偏移量",
	["page"] = "頁",
	["paste"] = "貼上",
	["point"] = "位置",
	["point_x"] = "水平定位點",
	["point_y"] = "垂直定位點",
	["position"] = "位置",
	["profile"] = "設定檔",
	["rename"] = "重命名",
	["reset"] = "重設",
	["size"] = "大小",
	["text"] = "文字",
	["width"] = "寬度",
}

L["page_names"] = {
	["auras"] = "光環",
	["bossmod"] = "首領模組",
	["castbars"] = "施法條",
	["classpowers"] = "職業資源",
	["cvars"] = "CVars參數",
	["fade_rules"] = "淡出控制",
	["framesizes"] = "框架尺寸",
	["general"] = "一般",
	["healthbars"] = "血條",
	["nameonly"] = "名字模式",
	["text"] = "文字",
	["threat"] = "威脅",
}

L["titles"] = {
	["absorb_enable"] = "顯示吸收",
	["absorb_striped"] = "條紋吸收材質",
	["auras_cd_movable"] = "冷卻",
	["auras_centre"] = "置中對齊圖示",
	["auras_colour_long"] = "長時間",
	["auras_colour_medium"] = "中等時間",
	["auras_colour_short"] = "短時間",
	["auras_count_movable"] = "計數",
	["auras_enabled"] = "啟用",
	["auras_filtering_sep"] = "過濾",
	["auras_hide_all_other"] = "隱藏他人施放的所有光環",
	["auras_icon_minus_size"] = "圖示尺寸(次要)",
	["auras_icon_normal_size"] = "圖示尺寸(標準)",
	["auras_icon_squareness"] = "長寬比例",
	["auras_icons_sep"] = "圖示",
	["auras_kslc_hint"] = "KuiSpellListConfig（可在Curse上取得）可以讓你編輯法術黑名單和白名單，任何單位所施放的法術都可以放入自訂清單。",
	["auras_offset"] = "垂直位置",
	["auras_on_personal"] = "在個人資源上顯示",
	["auras_pulsate"] = "閃爍圖示",
	["auras_purge_opposite"] = "淨化在另一側",
	["auras_purge_size"] = "圖示尺寸(淨化)",
	["auras_show_all_self"] = "顯示自身施放的所有光環",
	["auras_show_purge"] = "顯示淨化",
	["auras_side"] = "側邊",
	["auras_sort"] = "排序方式",
	["auras_time_threshold"] = "計時器閾值",
	["bar_animation"] = "血條動畫",
	["bar_texture"] = "血條材質",
	["bossmod_clickthrough"] = "名條自動顯示時啟用點擊穿透",
	["bossmod_control_visibility"] = "允許首領警報插件控制友方名條的能見度",
	["bossmod_enable"] = "啟用首領提醒插件交流",
	["bossmod_icon_size"] = "圖示大小",
	["bossmod_x_offset"] = "水平位置",
	["bossmod_y_offset"] = "垂直位置",
	["bot_vertical_offset"] = "等級血量垂直偏移",
	["castbar_animate"] = "動畫",
	["castbar_animate_change_colour"] = "變更顏色",
	["castbar_colour"] = "施法條顏色",
	["castbar_detach"] = "分離",
	["castbar_detach_combine"] = "法術圖示疊加",
	["castbar_detach_nameonly"] = "在名字模式下顯示",
	["castbar_enable"] = "啟用",
	["castbar_height"] = "施法條高度",
	["castbar_icon"] = "顯示法術圖示",
	["castbar_icon_side"] = "法術圖示何側",
	["castbar_name"] = "顯示法術名稱",
	["castbar_name_vertical_offset"] = "法術名稱垂直偏移量",
	["castbar_shield"] = "顯示盾牌圖示",
	["castbar_showall"] = "顯示所有單位的施法條",
	["castbar_showenemy"] = "顯示敵方施法條",
	["castbar_showfriend"] = "顯示友方施法條",
	["castbar_showpersonal"] = "顯示自身施法條",
	["castbar_unin_colour"] = "打斷免疫顏色",
	["class_colour_enemy_names"] = "敵方姓名職業著色",
	["class_colour_friendly_names"] = "友方姓名職業著色",
	["classpowers_bar_height"] = "資源條高度",
	["classpowers_bar_width"] = "資源條寬度",
	["classpowers_colour"] = "圖示顏色",
	["classpowers_colour_inactive"] = "閒置狀態",
	["classpowers_colour_overflow"] = "資源溢出",
	["classpowers_colour_supercharged"] = 'Super charged colour',
	["classpowers_colour_supercharged_active"] = 'Super charged colour active',
	["classpowers_colour_additional_active"] = 'Bonus combo colour',
	["classpowers_enable"] = "啟用",
	["classpowers_on_enemies"] = "在敵對",
	["classpowers_on_friends"] = "在友方",
	["classpowers_on_target"] = "顯示於目標",
	["classpowers_size"] = "圖示大小",
	["clickthrough_enemy"] = "敵方",
	["clickthrough_friend"] = "友方",
	["clickthrough_self"] = "個人",
	["clickthrough_sep"] = "點擊穿透",
	["colour_absorb"] = "吸收塗層",
	["colour_enemy_class"] = "敵方玩家職業著色",
	["colour_enemy_pet"] = "寵物",
	["colour_enemy_player"] = "玩家",
	["colour_friendly"] = "友善",
	["colour_friendly_pet"] = "寵物",
	["colour_hated"] = "敵對",
	["colour_neutral"] = "中立",
	["colour_player"] = "玩家",
	["colour_player_class"] = "友方玩家職業著色",
	["colour_self"] = "自己",
	["colour_self_class"] = "自身職業著色",
	["colour_tapped"] = "無拾取權",
	["combat_friendly"] = "戰鬥動作：友方",
	["combat_hostile"] = "戰鬥動作：敵方",
	["copy_profile_label"] = "鍵入新設定檔的名稱",
	["copy_profile_title"] = "複製設定檔",
	["cvar_clamp_bottom"] = "固定底部",
	["cvar_clamp_top"] = "固定頂部",
	["cvar_disable_alpha"] = "停用淡出",
	["cvar_disable_scale"] = "停用縮放",
	["cvar_enable"] = "允許Kui Nameplates修改CVars參數",
	["cvar_max_distance"] = "最大可視距離",
	["cvar_name_only"] = "隱藏預設生命條",
	["cvar_occluded_mult"] = "視線外透明度",
	["cvar_overlap_v"] = "垂直堆疊間距",
	["cvar_personal_show_always"] = "總是顯示個人名條",
	["cvar_personal_show_combat"] = "戰鬥中顯示個人名條",
	["cvar_personal_show_target"] = "當有目標時顯示個人名條",
	["cvar_self_alpha"] = "自身透明度",
	["cvar_self_clamp_bottom"] = "自身固定底部",
	["cvar_self_clamp_top"] = "自身固定頂部",
	["cvar_show_friendly_npcs"] = "總是顯示友方NPC名條",
	["dd_auras_sort_index"] = "光環索引",
	["dd_auras_sort_time"] = "剩餘時間",
	["dd_bar_animation_cutaway"] = "切除",
	["dd_bar_animation_smooth"] = "平滑",
	["dd_combat_toggle_hide"] = "戰鬥隱藏，脫戰顯示",
	["dd_combat_toggle_nothing"] = "不做什麼",
	["dd_combat_toggle_show"] = "戰鬥顯示，脫戰隱藏",
	["dd_font_style_monochrome"] = "點陣字描邊",
	["dd_font_style_none"] = "無",
	["dd_font_style_outline"] = "描邊",
	["dd_font_style_shadow"] = "陰影",
	["dd_font_style_shadowandoutline"] = "陰影+描邊",
	["dd_health_text_blank"] = "空白",
	["dd_health_text_current"] = "當前值",
	["dd_health_text_current_deficit"] = "當前 + 損血",
	["dd_health_text_current_percent"] = "當前 + 百分比",
	["dd_health_text_deficit"] = "損失值",
	["dd_health_text_maximum"] = "最大值",
	["dd_health_text_percent"] = "百分比",
	["delete_profile_label"] = "刪除設定檔 |cffffffff%s|r?",
	["delete_profile_title"] = "刪除設定檔",
	["execute_auto"] = "自動偵測斬殺血量",
	["execute_colour"] = "斬殺著色",
	["execute_enabled"] = "啟用斬殺著色",
	["execute_percent"] = "斬殺閾值",
	["execute_sep"] = "斬殺階段",
	["fade_all"] = "預設淡出",
	["fade_avoid_casting_friendly"] = "避開施法中友方",
	["fade_avoid_casting_hostile"] = "避開施法中敵方",
	["fade_avoid_casting_interruptible"] = "可中斷的",
	["fade_avoid_casting_uninterruptible"] = "不可中斷",
	["fade_avoid_combat"] = "避開戰鬥中",
	["fade_avoid_execute_friend"] = "避開低血量友方",
	["fade_avoid_execute_hostile"] = "避開低血量敵方",
	["fade_avoid_mouseover"] = "滑鼠指向",
	["fade_avoid_nameonly"] = "避開名字模式",
	["fade_avoid_raidicon"] = "避開團隊標記",
	["fade_avoid_sep"] = "不淡出...",
	["fade_avoid_tracked"] = "避開已追蹤或戰鬥中",
	["fade_conditional_alpha"] = "透明條件",
	["fade_friendly_npc"] = "淡出友方NPC",
	["fade_neutral_enemy"] = "淡出中立單位",
	["fade_non_target_alpha"] = "非目標透明度",
	["fade_speed"] = "淡出動畫速度",
	["fade_untracked"] = "淡出非追蹤單位",
	["font_face"] = "文字字型",
	["font_size_normal"] = "一般字型大小",
	["font_size_small"] = "小字型大小",
	["font_style"] = "文字樣式",
	["frame_glow_size"] = "光暈尺寸",
	["frame_glow_size_shadow"] = "陰影大小",
	["frame_glow_size_target"] = "目標光暈大小",
	["frame_glow_size_threat"] = "威脅光暈大小",
	["frame_glow_threat"] = "顯示威脅光暈",
	["frame_height_personal"] = "個人資源高度",
	["frame_minus_size"] = "僕從大小",
	["frame_padding_x"] = "點擊框填充寬度",
	["frame_padding_y"] = "點擊框填充高度",
	["frame_target_size"] = "目標大小",
	["frame_width_personal"] = "個人資源寬度",
	["global_scale"] = "全局縮放",
	["glow_as_shadow"] = "顯示陰影光暈",
	["guild_text_npcs"] = "顯示NPC頭銜",
	["guild_text_players"] = "顯示玩家公會",
	["health_text"] = "顯示血量",
	["health_text_friend_dmg"] = "損血友方",
	["health_text_friend_max"] = "滿血友方",
	["health_text_hostile_dmg"] = "損血敵方",
	["health_text_hostile_max"] = "滿血敵方",
	["health_text_percent_symbol"] = "顯示百分比符號",
	["health_text_sep"] = "血量文字",
	["hide_names"] = "隱藏非追蹤單位名稱",
	["ignore_uiscale"] = "像素校正",
	["level_nameonly"] = "顯示等級",
	["level_text"] = "顯示等級",
	["mouseover_glow"] = "滑鼠指向光暈",
	["mouseover_glow_colour"] = "滑鼠指向光暈顏色",
	["mouseover_highlight"] = "鼠標停留高亮",
	["name_colour_npc_friendly"] = "友方",
	["name_colour_npc_hostile"] = "敵方",
	["name_colour_npc_neutral"] = "中立",
	["name_colour_player_friendly"] = "友方玩家",
	["name_colour_player_hostile"] = "敵方玩家",
	["name_colour_sep"] = "姓名文字顏色",
	["name_colour_white_in_bar_mode"] = "可見的生命條中名字為白色",
	["name_text"] = "顯示名稱",
	["name_vertical_offset"] = "名字垂直偏移",
	["nameonly"] = "啟用名字模式",
	["nameonly_all_enemies"] = "使用在敵對",
	["nameonly_combat_friends"] = "戰鬥中的友方",
	["nameonly_combat_hostile"] = "戰鬥中的敵方",
	["nameonly_combat_hostile_player"] = "對於你",
	["nameonly_damaged_enemies"] = "受到傷害的敵方",
	["nameonly_damaged_friends"] = "在受損傷的友方",
	["nameonly_enemies"] = [=[敵人NPC
]=],
	["nameonly_friendly_players"] = "友方玩家",
	["nameonly_friends"] = "友方NPC",
	["nameonly_health_colour"] = "血量著色",
	["nameonly_hostile_players"] = "敵對玩家",
	["nameonly_neutral"] = "在中立單位",
	["nameonly_no_font_style"] = "無字型描邊",
	["nameonly_target"] = "使用在目標",
	["nameonly_visibility_sep"] = "使用名字模式在...",
	["new_profile"] = "新設定檔...",
	["new_profile_label"] = "輸入設定檔名稱",
	["paste_page_label"] = "複製|cffffffff%s|r設置從|cffffffff%s|r到|cffffffff%s|r嗎？",
	["powerbar_height"] = "能量條高度",
	["profile"] = "設定檔",
	["reaction_colour_sep"] = "血量條顏色",
	["rename_profile_label"] = "將|cffffffff%s重新命名",
	["rename_profile_title"] = "重新命名設定檔",
	["reset_page_label"] = "重設所有在|cffffffff%s|r的選項嗎？",
	["reset_profile_label"] = "重置設定檔|cffffffff%s|r？",
	["reset_profile_title"] = "重置設定檔",
	["show_quest_icon"] = "任務圖示",
	["show_raid_icon"] = "團隊圖示",
	["state_icons"] = "顯示狀態圖示",
	["tank_mode"] = "啟用",
	["tankmode_colour_sep"] = "坦克模式顏色",
	["tankmode_force_enable"] = "強制坦克模式",
	["tankmode_force_offtank"] = "強制換坦偵測",
	["tankmode_glow_colour_sep"] = "威脅顏色",
	["tankmode_other_colour"] = "副坦第二仇恨",
	["tankmode_tank_colour"] = "當前坦仇恨穩定",
	["tankmode_tank_glow_colour"] = "坦住",
	["tankmode_trans_colour"] = "轉換",
	["tankmode_trans_glow_colour"] = "轉換",
	["target_arrows"] = "顯示目標箭頭",
	["target_arrows_size"] = "箭頭尺寸",
	["target_glow"] = "顯示目標光暈",
	["target_glow_colour"] = "目標光暈顏色",
	["threat_brackets"] = "顯示威脅括號",
	["title_text_players"] = "顯示玩家頭銜",
	["use_blizzard_personal"] = "忽略個人資源",
	["use_blizzard_powers"] = "顯示預設職業能量",
	["version"] = "%s 由%s 在%s|n版本 %s",
}

L["tooltips"] = {
	["absorb_enable"] = "在血條上顯示吸收盾",
	["absorb_striped"] = "在吸收盾上使用條紋材質。如果未勾選，繼承血條材質。",
	["auras_cd_size"] = "設置為0以繼承 '普通' 文字大小",
	["auras_colour_long"] = "超過20秒",
	["auras_colour_medium"] = "不到20秒",
	["auras_colour_short"] = "5秒以內",
	["auras_count_size"] = "設置為0以繼承 '小' 文字大小",
	["auras_enabled"] = "在名條上顯示你施放的光環：友方顯示增益，敵方顯示減益",
	["auras_hide_all_other"] = "不顯示任何他人施放的光環（如控場和緩速）。|n|n請注意：KuiSpellList的生效優先級高於此選項，所以你仍然可以在勾選此選項的情況下用|cffffff88KuiSpellListConfig|r（可在Curse上取得）自訂特定法術的顯示。",
	["auras_icon_minus_size"] = "次要單位名條的圖示大小",
	["auras_icon_normal_size"] = "標準單位名條的圖示大小",
	["auras_icon_squareness"] = "光環圖示的長寬比例，設為1代表正方形",
	["auras_on_personal"] = "如果啟用，在個人資源上顯示你的光環",
	["auras_pulsate"] = "快要結束時閃爍圖標",
	["auras_show_all_self"] = "顯示你施放的所有光環，而非只顯示暴雪預設的重要光環。|n|n請注意：KuiSpellList的生效優先級高於此選項，所以你仍然可以在勾選此選項的情況下用|cffffff88KuiSpellListConfig|r（可在Curse上取得）自訂特定法術的顯示。",
	["auras_show_purge"] = "在敵方顯示你可以偷取、驅散或淨化的增益",
	["auras_time_threshold"] = "當光環的剩餘秒數少於這個數值時，顯示倒數文字。設為-1可使倒數計時總是顯示",
	["bar_animation"] = "血量／能量條變化的動畫方式",
	["bar_texture"] = "狀態條使用的材質（由LibSharedMedia提供）",
	["bossmod_clickthrough"] = "當友方血條自動顯示的時候禁用點擊",
	["bossmod_control_visibility"] = "如果在首領戰中有使用於光環允許首領提醒插件顯示名條。",
	["bossmod_enable"] = "支援的首領提醒插件可以與KNP交流，顯示特定首領戰的光環並在名調上描繪線條。",
	["castbar_animate"] = "當施法結束時淡出施法條。",
	["castbar_animate_change_colour"] = "當施法結束時變更施法條顏色，這樣可以更好分辨成功、中止、被打斷施法的區別。",
	["castbar_colour"] = "施法條顏色。|n|n如果有啟用動畫，也用於表示成功施放。",
	["castbar_enable"] = "啟用施法條",
	["castbar_name_vertical_offset"] = "法術名稱文字的垂直偏移量",
	["castbar_shield"] = "在免疫打斷的施法條上顯示盾牌圖示",
	["castbar_showall"] = "在所有名條上顯示施法條",
	["castbar_showenemy"] = "顯用敵方施法條",
	["castbar_showfriend"] = "顯示友方施法條，注意：啟用名字模式時不會顯示施法條",
	["castbar_showpersonal"] = "如果啟用個人資源，在其上顯示自身施法條",
	["castbar_unin_colour"] = "著色無法被打斷的施法條",
	["classpowers_enable"] = "顯示副資源，像是連擊點、聖能等等。",
	["classpowers_on_target"] = "將副資源顯示於目標，而非依附個人資源",
	["clickthrough_enemy"] = "禁止點擊個人資源",
	["clickthrough_friend"] = "禁止點擊友方名條",
	["clickthrough_self"] = "禁止點擊個人資源",
	["colour_friendly_pet"] = "請注意：友方玩家寵物一般不會顯示名條",
	["colour_player"] = "其他友方玩家名條的顏色",
	["colour_self"] = "個人資源的血條顏色",
	["colour_self_class"] = "以職業顏色著色個人資源",
	["combat_friendly"] = "進入與離開戰鬥時在友方框架上採取的動作。",
	["combat_hostile"] = "進入與離開戰鬥時在敵方框架上採取的動作。",
	["cvar_clamp_bottom"] = "|cffffcc00nameplateOtherBottomInset|nnameplateLargeBottomInset|r",
	["cvar_clamp_top"] = "|cffffcc00nameplateOtherTopInset|nnameplateLargeTopInset|r|n|n名條與螢幕頂部邊緣的接近程度，其中0表示邊緣。設置為-0.1以禁用在螢幕頂部的固定。|n|n固定只會影響您當前的目標。",
	["cvar_disable_alpha"] = "|cffffcc00nameplateMinAlpha|nnameplateMaxAlpha|nnameplateSelectedAlpha|r|n|n禁用名條透明度CVar參數（以下內容除外），以免干擾KNP的淡出規則。",
	["cvar_disable_scale"] = "|cffffcc00nameplateMinScale|nnameplateMaxScale|nnameplateLargerScale|nnameplateSelectedScale|nnameplateSelfScale|r|n|n禁用破壞像素校正的名條距離縮放CVar參數。",
	["cvar_enable"] = "啟用後，Kui Nameplates會套用本頁所設定的CVar參數，並鎖定數值。|n|n請注意：停用此選項只會使KNP停止控制與鎖定CVar參數，而非重置數值。",
	["cvar_max_distance"] = "|cffffcc00nameplateMaxDistance|r|n|n名條最大可視的距離（不包含當前目標）。",
	["cvar_name_only"] = "|cffffcc00nameplateShowOnlyNames|r|n|n在友方名條無法被插件修改的情形下，隱藏預設名條的生命條。",
	["cvar_occluded_mult"] = "|cffffcc00nameplateOccludedAlphaMult|r|n|n透明度乘數適用於不在玩家視線內的名條。",
	["cvar_overlap_v"] = "|cffffcc00nameplateOverlapV|r|n|n名條與名條之間的垂直間距（只有在暴雪的介面選項中將排列類型設定為「堆疊名條」才有效）。",
	["cvar_personal_show_always"] = "|cffffcc00nameplatePersonalShowAlways|r",
	["cvar_personal_show_combat"] = "|cffffcc00nameplatePersonalShowInCombat|r",
	["cvar_personal_show_target"] = "|cffffcc00nameplatePersonalShowWithTarget|r|n|n如果你選取了一個可攻擊目標，則顯示個人名條。",
	["cvar_self_alpha"] = "|cffffcc00nameplateSelfAlpha|r|n|n個人名條的最大透明度。",
	["cvar_self_clamp_bottom"] = "|cffffcc00nameplateSelfBottomInset|r",
	["cvar_self_clamp_top"] = "|cffffcc00nameplateSelfTopInset|r",
	["cvar_show_friendly_npcs"] = "|cffffcc00nameplateShowFriendlyNPCs|r",
	["execute_auto"] = "自動偵測你的天賦專精所需的斬殺閾值，對於無斬殺的角色預設為20%",
	["execute_colour"] = "斬殺階段使用的顏色",
	["execute_enabled"] = "當單位進入斬殺階段時，重新著色血條",
	["execute_percent"] = "手動設定斬殺階段血量閾值",
	["fade_all"] = "預設狀態下淡出所有框架",
	["fade_avoid_execute_friend"] = "不要淡出血量處於斬殺階段的友方名條（在「血量條」頁面設置）",
	["fade_avoid_execute_hostile"] = "不要淡出血量處於斬殺階段的敵方名條（在「血量條」頁面設置）",
	["fade_avoid_tracked"] = "不淡出已追蹤或是正在與你戰鬥中的單位名條。|n透過改變 Esc > 介面 > 名稱 > 「NPC名稱」的選項來設定是否進行追蹤。",
	["fade_conditional_alpha"] = "當符合以下條件時非透明框架將會淡出",
	["fade_friendly_npc"] = "預設淡出友方NPC名條（包含名字模式）",
	["fade_neutral_enemy"] = "預設淡出可攻擊的中立單位名條（包含名字模式）",
	["fade_non_target_alpha"] = "當你有目標時其他非透明框架將會淡出。|n如果設置為0，就算名條不可見仍可點擊，插件無法隨意停用名條點擊框。",
	["fade_speed"] = "框架淡出的速度，1是最慢的，0是立刻淡出",
	["fade_untracked"] = "淡出非追蹤名條（包含名字模式）。|n透過改變 Esc > 介面 > 名稱中的「NPC名稱」的選項來設定是否進行追蹤",
	["font_face"] = "名條所使用的全局字型（由LibSharedMedia提供）",
	["font_size_normal"] = "標準字型大小 （名稱等）",
	["font_size_small"] = "小字型大小（商人、法術名稱等等）",
	["frame_glow_threat"] = "以光暈的顏色變化來指示威脅狀態",
	["frame_minus_size"] = "標記為“僕從”的小怪的替代框架大小",
	["frame_target_size"] = "當前目標的替代框架大小",
	["frame_width_personal"] = "個人資源條的寬度（若要啟用，勾選 Esc > 介面 > 名稱 > 顯示個人資源）",
	["global_scale"] = "根據此數字縮放所有名條(遵從像素網格)",
	["guild_text_npcs"] = "顯示NPC的頭銜，像是軍需官...等等。",
	["health_text_friend_dmg"] = "友方玩家損血時的血量文字格式",
	["health_text_friend_max"] = "友方玩家滿血時的血量文字格式",
	["health_text_hostile_dmg"] = "敵方玩家損血時的血量文字格式",
	["health_text_hostile_max"] = "敵方玩家滿血時的血量文字格式",
	["hide_names"] = "你可以透過改變 Esc > 介面 > 名稱 > 「NPC名稱」的選項來設定是否進行追蹤。注意：此設定在名字模式不生效",
	["ignore_uiscale"] = "修復與界面縮放相關的像素對齊問題。通過調整/knp > 框架尺寸 > 全局縮放來補償大小差異。|n|n即使您並沒有啟用UI縮放這也是必要的。",
	["name_colour_white_in_bar_mode"] = "NPC與玩家名字使用白色著色（除非啟用職業著色）。|n|n如果此選項啟用，以下顏色只適用於名字模式。",
	["nameonly_all_enemies"] = "只套用在敵對NPC",
	["nameonly_combat_hostile"] = "請注意，這不適用於訓練假人或其他沒有威脅數據的單位",
	["nameonly_health_colour"] = "以部份著色的方式來顯示血量百分比",
	["nameonly_no_font_style"] = "使用名字模式時，不使用字體描邊（將字型樣式設為空）",
	["nameonly_target"] = "也在當前目標使用名字模式",
	["powerbar_height"] = "能量條的高度。不能超過名條框架高度",
	["reload_hint"] = "需要重載UI。",
	["state_icons"] = "在首領與精英單位上顯示圖示（啟用「顯示等級文字」時隱藏）",
	["tank_mode"] = "使用坦克專精時，重新著色你正在坦住的單位血條顏色",
	["tankmode_force_enable"] = "總是使用坦克模式，不管你是否處於坦克專精",
	["tankmode_force_offtank"] = "著色被團隊中其他坦克所坦住的血條，即使你目前並非坦克專精",
	["tankmode_other_colour"] = "當其他坦克接力坦住時的血條顏色。|n|n只對坦克專精生效，並且只對同個團隊中、職責要設定為坦克的角色生效。",
	["tankmode_tank_colour"] = "穩定坦住時的血條顏色",
	["tankmode_trans_colour"] = "獲得或失去仇恨時的血條顏色",
	["target_indicators"] = "顯示在當前目標周圍的指標。它們繼承了上面設置的目標光暈顏色。",
	["use_blizzard_personal"] = "不要美化個人名條或其職業能量。",
}

